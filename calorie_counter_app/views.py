...
class MealsRecordCreate(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        try:
            daily_record = DailyRecord.objects.get(
                date=timezone.now().date(), user=self.request.user)
...
class FoodItemRecordView(APIView):
    ...
    def post(self, request):
        ...
        food_item_instance = FoodItem.objects.filter(
            name__icontains=food_item).first()
        try:
            daily_record = DailyRecord.objects.get(
                date=timezone.now().date(), user=self.request.user)
        except DailyRecord.DoesNotExist:
            daily_record = DailyRecord.objects.create(
                date=timezone.now().date(), user=self.request.user)
        FoodItemRecord.objects.create(
            food_item=food_item_instance, daily_record=daily_record, amount=amount)
...
class ActivityRecordView(APIView):
    ...
    def post(self, request):
        ...
        activity_instance = Activity.objects.filter(
            name__icontains=activity).first()
        try:
            daily_record = DailyRecord.objects.get(
                date=timezone.now().date(), user=self.request.user)
        except DailyRecord.DoesNotExist:
        ...
        ActivityRecord.objects.create(
            activity=activity_instance, daily_record=daily_record, duration=duration)
...
class RecordList(APIView):
    ...
    def get(self, request):
        daily_activity_durations = ActivityRecord.objects.values(
            'activity__name', 'daily_record__date').annotate(duration_sum=Sum('duration'))
        daily_food_item_amount = FoodItemRecord.objects.values(
            'food_item__name', 'daily_record__date').annotate(duration_sum=Sum('amount'))
        monthly_activity_duration = ActivityRecord.objects.annotate(month=ExtractMonth('daily_record__date'), year=ExtractYear(
            'daily_record__date')).values('month', 'year', 'activity__name').annotate(sum_duration=Sum('duration'))
        monthly_food_item_amount = FoodItemRecord.objects.annotate(month=ExtractMonth('daily_record__date'), year=ExtractYear(
            'daily_record__date')).values('month', 'year', 'food_item__name').annotate(sum_amount=Sum('amount'))
